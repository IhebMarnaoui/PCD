...
app.config[
    'MONGODB_HOST'] = "mongodb+srv://iheb:ihebiheb@cluster0.xdwy6.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
db = MongoEngine()
db.init_app(app)
...
def graphe(company):
    ...
    last_output_arr = {"AAPL": list(IT_Companies[0]), "FB": list(IT_Companies[1]), "MSFT": list(IT_Companies[2]),
                       "HPQ": list(IT_Companies[3]), "CSCO": list(IT_Companies[4]), "ADM": list(IT_Companies[5]),
                       "IBM": list(IT_Companies[6]), "QCOM": list(IT_Companies[7])}
    if company == "IBM":
        future_days = 7
        for i in range(len(IT_Companies)):
            j = 0
...
def add():
    ...
    user = users(email=compte['email'], pwd=compte['pwd'], name=compte['name'], lastname=compte['last_name'])
    user.save()
    return 'ok'
...
def login():
    compte = json.loads(request.data)
    if users.objects(email=compte['email']).first():
        p = users.objects(email=compte['email']).first()
        if (p['pwd'] == compte['pwd']):
            return jsonify({"flag": 1})
        else:
            return jsonify({"flag": 0, "msg": "wrong pwd"})
    else:
        return jsonify({"flag": 0, "msg": "wrong email"})
